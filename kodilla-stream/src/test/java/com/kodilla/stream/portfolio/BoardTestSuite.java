...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        double averageNumberOfDaysOfWorkingTaskFromStartToNow = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated).mapToDouble( n -> n.getDayOfYear())
                .map(n -> LocalDate.now().getDayOfYear() - n)
                .average()
                .getAsDouble();

        assertEquals(10,averageNumberOfDaysOfWorkingTaskFromStartToNow);
    }
